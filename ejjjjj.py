...
from src.utils import guardar_objeto, evaluar_modelos
    ...
    def iniciar_entrenador_modelo(self, train_array, test_array):
        try:
            ...
            params = {
                ...
                "Gradient Boosting": {"learning_rate": [0.1, 0.01], "subsample": [0.6, 0.7, 0.8]},
                "Linear Regression": {},
                "XGBRegressor": {"learning_rate": [0.1, 0.01], "n_estimators": [100, 200]},
                "CatBoosting Regressor": {"depth": [6, 8, 10], "learning_rate": [0.01, 0.05]},
                "AdaBoost Regressor": {"learning_rate": [0.1, 0.01], "n_estimators": [50, 100]}
            }

            reporte_modelos: dict = evaluar_modelos(X_train, y_train, X_test, y_test, modelos, params)
            ...
            if mejor_puntaje < 0.6:
                raise ExcepcionPersonalizada("No se encontró un modelo con rendimiento aceptable")

            logging.info(f"Mejor modelo encontrado: {mejor_nombre_modelo} con R2 de {mejor_puntaje}")
